...
class InvalidateEnmSshKeyTaskTest extends CdiSpecification {

    private String NODE_NAME = "NODE"

    def 'constructor empty'() {
        given:
        def task = new InvalidateEnmSshKeyTask()
        expect:
        task.getTaskType() == WorkflowTaskType.INVALIDATE_ENM_SSH_KEY
        task.getShortDescription() == InvalidateEnmSshKeyTask.SHORT_DESCRIPTION
        task.getNode() == null
    ...
    def 'constructor by node'() {
        given:
        def task = new InvalidateEnmSshKeyTask(NODE_NAME)
        expect:
        task.getTaskType() == WorkflowTaskType.INVALIDATE_ENM_SSH_KEY
        task.getShortDescription() == InvalidateEnmSshKeyTask.SHORT_DESCRIPTION
        task.getNode() != null
