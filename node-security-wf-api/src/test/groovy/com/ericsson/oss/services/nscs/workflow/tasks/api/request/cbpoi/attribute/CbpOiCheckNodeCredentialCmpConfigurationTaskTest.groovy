...
class CbpOiCheckNodeCredentialCmpConfigurationTaskTest extends CdiSpecification {

    def "no-args constructor" () {
        given:
        when: "instantiating using no-arg constructor"
        def CbpOiCheckNodeCredentialCmpConfigurationTask task = new CbpOiCheckNodeCredentialCmpConfigurationTask()
        then: "task should not be null"
        task != null
        and: "task type should be CBP_OI_CHECK_CMP_CONFIG"
        task.getTaskType() == WorkflowTaskType.CBP_OI_CHECK_NODE_CREDENTIAL_CMP_CONFIG
        and: "node FDN should be null"
        task.getNodeFdn() == null
        and: "short description should be the task short description"
        task.getShortDescription() == CbpOiCheckNodeCredentialCmpConfigurationTask.SHORT_DESCRIPTION
    ...
    def "fdn constructor" () {
        ...
        def CbpOiCheckNodeCredentialCmpConfigurationTask task = new CbpOiCheckNodeCredentialCmpConfigurationTask(fdn)
        then: "task should not be null"
        task != null
        and: "task type should be CBP_OI_CHECK_CMP_CONFIG"
        task.getTaskType() == WorkflowTaskType.CBP_OI_CHECK_NODE_CREDENTIAL_CMP_CONFIG
