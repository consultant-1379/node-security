...
class VerifySshKeyTaskTest extends CdiSpecification {

    private String NODE_NAME = "NODE"

    def 'constructor empty'() {
        given:
        def task = new VerifySshKeyTask()
        expect:
        task.getTaskType() == WorkflowTaskType.VERIFY_SSH_KEY
        task.getShortDescription() == VerifySshKeyTask.SHORT_DESCRIPTION
        task.getNode() == null
    ...
    def 'constructor by node'() {
        given:
        def task = new VerifySshKeyTask(NODE_NAME)
        expect:
        task.getTaskType() == WorkflowTaskType.VERIFY_SSH_KEY
        task.getShortDescription() == VerifySshKeyTask.SHORT_DESCRIPTION
        task.getNode() != null
