...
class CbpOiPostOnlineEnrollmentTaskTest extends CdiSpecification {

    def "no-args constructor" () {
        given:
        when: "instantiating using no-arg constructor"
        def CbpOiPostOnlineEnrollmentTask task = new CbpOiPostOnlineEnrollmentTask()
        then: "task should not be null"
        task != null
        and: "task type should be CBP_OI_POST_ONLINE_ENROLLMENT"
        task.getTaskType() == WorkflowTaskType.CBP_OI_POST_ONLINE_ENROLLMENT
        and: "node FDN should be null"
        task.getNodeFdn() == null
        and: "short description should be the task short description"
        task.getShortDescription() == CbpOiPostOnlineEnrollmentTask.SHORT_DESCRIPTION
    ...
    def "fdn constructor" () {
        ...
        def CbpOiPostOnlineEnrollmentTask task = new CbpOiPostOnlineEnrollmentTask(fdn)
        then: "task should not be null"
        task != null
        and: "task type should be CBP_OI_POST_ONLINE_ENROLLMENT"
        task.getTaskType() == WorkflowTaskType.CBP_OI_POST_ONLINE_ENROLLMENT
