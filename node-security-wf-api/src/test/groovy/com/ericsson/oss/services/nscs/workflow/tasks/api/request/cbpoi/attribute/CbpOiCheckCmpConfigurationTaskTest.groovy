...
class CbpOiCheckCmpConfigurationTaskTest extends CdiSpecification {

    def "no-args constructor" () {
        given:
        when: "instantiating using no-arg constructor"
        def CbpOiCheckCmpConfigurationTask task = new CbpOiCheckCmpConfigurationTask()
        then: "task should not be null"
        task != null
        and: "task type should be CBP_OI_CHECK_CMP_CONFIG"
        task.getTaskType() == WorkflowTaskType.CBP_OI_CHECK_CMP_CONFIG
        and: "node FDN should be null"
        task.getNodeFdn() == null
        and: "short description should be the task short description"
        task.getShortDescription() == CbpOiCheckCmpConfigurationTask.SHORT_DESCRIPTION
    ...
    def "fdn constructor" () {
        ...
        def CbpOiCheckCmpConfigurationTask task = new CbpOiCheckCmpConfigurationTask(fdn)
        then: "task should not be null"
        task != null
        and: "task type should be CBP_OI_CHECK_CMP_CONFIG"
        task.getTaskType() == WorkflowTaskType.CBP_OI_CHECK_CMP_CONFIG
