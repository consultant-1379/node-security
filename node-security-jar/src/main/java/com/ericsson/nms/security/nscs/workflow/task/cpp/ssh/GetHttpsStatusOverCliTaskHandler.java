...
import com.ericsson.oss.services.nscs.workflow.tasks.api.request.cpp.ssh.GetHttpsStatusOverCliTask;
 ...
 *
 */
@WFTaskType(WorkflowTaskType.GET_HTTPS_STATUS_CLI)
@Local(WFTaskHandlerInterface.class)
public class GetHttpsStatusOverCliTaskHandler implements WFActionTaskHandler<GetHttpsStatusOverCliTask>, WFTaskHandlerInterface {
    ...
    @Override
    public void processTask(GetHttpsStatusOverCliTask task) {
        ...
        sshCommandJob.setCommandToExecute(CPPCommands.SECMODE_S.toString());
        sshCommandJob.setNodeAddress(networkElementSecurityFdn);

        nscsLogger.workFlowTaskHandlerOngoing(task,
                "Execution of command: " + CPPCommands.SECMODE_S.toString() + "on node: " + networkElementSecurityFdn);
        commandJobSender.send(sshCommandJob);

        nscsLogger.workFlowTaskHandlerFinishedWithSuccess(task,
                "Command [" + sshCommandJob + "] sent to the node with Success");
