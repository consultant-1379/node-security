...
@WFTaskType(WorkflowTaskType.CPP_ENABLE_HTTPS)
@Local(WFTaskHandlerInterface.class)
public class ActivateHttpsOnNodeTaskHandler implements WFActionTaskHandler<ActivateHttpsOnNodeTask>, WFTaskHandlerInterface {
    ...
    @Override
    public void processTask(final ActivateHttpsOnNodeTask task) {
        nscsLogger.workFlowTaskHandlerStarted(task);

        final NormalizableNodeReference normRef = readerService.getNormalizedNodeReference(task.getNode());
        if (normRef == null) {
            nscsLogger.workFlowTaskHandlerFinishedWithError(task, "Could not find normalized node reference. Can't send SSH command to node.");
        ...
        sshCommandJob.setCommandToExecute(CPPCommands.SECMODE_W_S.toString());
        sshCommandJob.setNodeAddress(networkElementSecurityFdn);

        nscsLogger.workFlowTaskHandlerOngoing(task, "Execution of command: " + CPPCommands.SECMODE_W_S.toString());
        commandJobSender.send(sshCommandJob);
        nscsLogger.workFlowTaskHandlerFinishedWithSuccess(task,
                "Command [" + sshCommandJob + "] sent with Success");
    }
}
