...
/**
 * <p>
 * Initiates the process to set the crlcheck attribute as activated(enable) on a set of nodes.
 * </p>
 * 
 * @author xkumkam
 *
...
@CommandType(NscsCommandType.CRL_CHECK_ENABLE)
@Local(CommandHandlerInterface.class)
public class CrlCheckEnableHandler implements CommandHandler<CrlCheckCommand>, CommandHandlerInterface {
    ...
    public static final String CRLCHECK_ENABLE_EXECUTED = "Successfully started a job for CRL Check enable operation. Perform 'secadm job get -j %s' to get progress info.";
    public static final String CRLCHECK_ENABLE_EXECUTED_DYN_ISSUE = "Successfully started a job for CRL Check enable operation for some node(s). Perform 'secadm job get -j %s' to get progress info of valid nodes. Invalid node details are given below :";
    public static final String CRLCHECK_ENABLE_NOT_EXECUTED = "Failed to start the job for CRL Check enable operation as all the provided node(s) are invalid. Invalid node details are given below :";
    public static final String CRLCHECK_ENABLE_WF_FAILED = " during CRL Check enable operation.";
    ...
    private static final String CRL_CHECK_STATUS = "ACTIVATED";
    ...
    public NscsCommandResponse process(final CrlCheckCommand command, final CommandContext context) throws NscsServiceException {
        ...
        if (areInputNodesValid) {
            try {
            	jobStatusRecord = cacheHandler.insertJob(NscsCommandType.CRL_CHECK_ENABLE);
                nscsLogger.info("All of the given input nodes are Valid.CrlCheckEnable workflow need to be executed.");
                ...
                throw new CrlCheckEnableOrDisableWfException(CRLCHECK_ENABLE_WF_FAILED);
            } finally {
                long endTime = System.currentTimeMillis();
                nscsLogger.info("Total elapsed time for CrlCheck Enable Handler: " + String.format("%.3f", (endTime - startTime) / 1000f));
            }
            responseMessageWithJobId = String.format(CRLCHECK_ENABLE_EXECUTED, jobStatusRecord.getJobId().toString());
            ...
            if (!validNodesList.isEmpty()) {
                ...
                try {
                	jobStatusRecord = cacheHandler.insertJob(NscsCommandType.CRL_CHECK_ENABLE);
                    nscsLogger.info("CrlCheckEnable workflow need to be executed for valid nodes.");
                    commandManager.executeCrlCheckWfs(validNodesList, command.getCertType(), CRL_CHECK_STATUS, jobStatusRecord);
                    responseMessageWithJobId = String.format(CRLCHECK_ENABLE_EXECUTED_DYN_ISSUE, jobStatusRecord.getJobId().toString());
                } catch (Exception ex) {
                    nscsLogger.error(ex.getMessage(), ex);
                    nscsLogger.commandHandlerFinishedWithError(command, ex.getMessage());
                    throw new CrlCheckEnableOrDisableWfException(CRLCHECK_ENABLE_WF_FAILED);
                ...
                response.setAdditionalInformation(CRLCHECK_ENABLE_NOT_EXECUTED);
                nscsLogger.commandHandlerFinishedWithError(command, CRLCHECK_ENABLE_NOT_EXECUTED);
                return response;
            }
