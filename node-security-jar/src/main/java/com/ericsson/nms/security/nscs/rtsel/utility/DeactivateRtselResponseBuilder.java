...
/**
 * This class is for to build the response object for DeactivateRtselHandler
 * 
 * @author xvadyas
 * 
 */
public class DeactivateRtselResponseBuilder {
         ...
         *            map of invalid nodes with error details. 
        ...
        response.setAdditionalInformation(RtselConstants.DEACTIVATE_RTSEL_FAILED_FOR_ALL_NODES);
            return response;
        }
        ...
        public NscsCommandResponse buildResponseForAllValidInputNodes(final JobStatusRecord jobStatusRecord) {
            String jobIdMessage = "";
            try {
            nscsLogger.info("All of the given input nodes are Valid. Deactivate Rtsel workflow need to be executed.");
            jobIdMessage = RtselConstants.DEACTIVATE_RTSEL_SUCCESS_FOR_ALL_NODES + ". Perform 'secadm job get -j " + jobStatusRecord.getJobId().toString() + "' to get progress information.";
            } catch (Exception ex) {
        ...
        nscsLogger.info("Some of the given input nodes are Valid. Deactivate Rtsel workflow need to be executed.");
        jobIdMessage = String.format(RtselConstants.DEACTIVATE_RTSEL_FAILED_FOR_SOME_NODES, jobStatusRecord.getJobId().toString());
            final NscsNameMultipleValueCommandResponse response = NscsCommandResponse.nameMultipleValue(RtselConstants.NO_OF_COLUMNS);
            response.add(ERROR_HEADER[0], Arrays.copyOfRange(ERROR_HEADER, 1, ERROR_HEADER.length));
            nscsLogger.info("invalidNodesErrorMap is : [{}]", invalidNodesErrorMap);
            for (java.util.Map.Entry<NodeReference, NscsServiceException> entry : invalidNodesErrorMap.entrySet()) {
