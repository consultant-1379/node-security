...
@WFTaskType(WorkflowTaskType.CPP_DISABLE_HTTPS)
@Local(WFTaskHandlerInterface.class)
public class DeactivateHttpsOnNodeTaskHandler implements WFActionTaskHandler<DeactivateHttpsOnNodeTask>, WFTaskHandlerInterface {
    ...
    @Override
    public void processTask(final DeactivateHttpsOnNodeTask task) {
        ...
        sshCommandJob.setCommandToExecute(CPPCommands.SECMODE_W_U.toString());
        sshCommandJob.setNodeAddress(networkElementSecurityFdn);

        nscsLogger.workFlowTaskHandlerOngoing(task, "Execution of command: " + CPPCommands.SECMODE_W_U.toString()
                + "on node: " + networkElementSecurityFdn);
        commandJobSender.send(sshCommandJob);
        nscsLogger.workFlowTaskHandlerFinishedWithSuccess(task,
                "Command [" + sshCommandJob + "] sent to the node [" + task.getNodeFdn() + "] with Success");
    }
}
