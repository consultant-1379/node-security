 ...
 */
public class SnmpAuthnoprivParamsValidator implements CommandValidator {
    ...
    public void validate(final NscsPropertyCommand command, final CommandContext context) throws NscsServiceException {
        logger.debug("Starting SnmpAuthnoprivParamsValidator with command type: {}", command.getCommandType());
        ...
        if (command instanceof NscsPropertyCommand) {
            ...
            // Check Mandatory auth_algo
            final SnmpAuthnopriv snmpAuthnoprivCommand = (SnmpAuthnopriv) command;
            if (snmpAuthnoprivCommand.getProperties().keySet().contains(SnmpAuthnopriv.AUTH_ALGO_PARAM)
                    && !SnmpAuthnopriv.getAuthProtocolList().contains(snmpAuthnoprivCommand.getAuthAlgo())) {
                final String errmsg = String.format("" + "Invalid argument for parameter %s",
                        SnmpAuthnopriv.AUTH_ALGO_PARAM + ". Accepted arguments are " + SnmpAuthnopriv.getAuthProtocolList());
                logger.error(errmsg);
    ...
    private boolean missingAttributes(final NscsPropertyCommand command, final NormalizableNodeReference normNodeRef) {
        ...
        if (isValidCommand) {
            final List<String> expectedAttributeKeys = SnmpCommandHelper.getExpectedSnmpAuthnoprivParams();
