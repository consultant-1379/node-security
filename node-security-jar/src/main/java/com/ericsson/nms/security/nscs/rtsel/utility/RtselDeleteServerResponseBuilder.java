...
/**
 * This class build the response object for RtselDeleteServerHandler
 * 
 * @author xchowja
 *
 */
public class RtselDeleteServerResponseBuilder {
    ...
    public NscsNameMultipleValueCommandResponse buildResponseForAllInvalidInputNodes(final RtselCommand command, final Map<NodeReference, NscsServiceException> invalidNodesErrorMap) {
        ...
        response.setAdditionalInformation(RtselConstants.RTSEL_DELETE_SERVER_FAILED_FOR_ALL_NODES);
        return response;
    }
    ...
    public NscsCommandResponse buildResponseForAllValidInputNodes(final JobStatusRecord jobStatusRecord) {
        String jobIdMessage = "";
        try {
            nscsLogger.info("All of the given input nodes are Valid. Rtsel delete server workflow need to be executed.");
            jobIdMessage = RtselConstants.RTSEL_DELETE_SERVER_SUCCESS_FOR_ALL_NODES + ". Perform 'secadm job get -j " + jobStatusRecord.getJobId().toString() + "' to get progress information.";

            nscsLogger.info("RtselDeleteServerResponseBuilder {}", jobIdMessage);
        } catch (Exception ex) {
    ...
    public NscsCommandResponse buildResponseForPartialValidInputNodes(final JobStatusRecord jobStatusRecord, final Map<NodeReference, NscsServiceException> invalidNodesErrorMap) {
        ...
        nscsLogger.info("Some of the given input nodes are Valid. Rtsel delete server workflow need to be executed.");
        jobIdMessage = String.format(RtselConstants.RTSEL_DELETE_SERVER_FAILED_FOR_SOME_NODES, jobStatusRecord.getJobId().toString());
