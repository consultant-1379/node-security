...
import com.ericsson.oss.services.nscs.workflow.tasks.log.LogTimeoutTask;

@WFTaskType(WorkflowTaskType.LOG_TIMEOUT)
@Local(WFTaskHandlerInterface.class)
public class LogTimeoutTaskHandler implements WFActionTaskHandler<LogTimeoutTask>, WFTaskHandlerInterface {
    ...
    @Override
    public void processTask(final LogTimeoutTask task) {
        ...
        final String additionalInfo = String.format("Workflow timed-out at step " + activationStep + " with jobID: "
        ...
        if (wfStatusId != null) {
            ...
            wfResult.setMessage(((message != null && !message.isEmpty()) ? message + " [" : "[") + "TIMEOUT]");
            wfResult.setWfParams(new HashMap<String, Object>());
            cacheHandler.updateWorkflow(wfResult);
            nscsLogger.info("LogTimeoutTaskHandler ready to update cache for wfResult [{}]", wfResult);
