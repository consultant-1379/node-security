...
class UpdateKeyGeneratorHandlerFcTest extends CdiSpecification{

    private static final String ALL_VALID_NODES_FORMAT = "Successfully started a job for updating SSH key."

    @ObjectUnderTest
    UpdateKeyGeneratorHandler updateKeyGeneratorHandler
    ...
    def "when update ssh key command is called and all node are valid it returns a proper response message"() {
        given:
        ...
        when:
        def response = (NscsMessageCommandResponse)updateKeyGeneratorHandler.process(command, context)
        then:
