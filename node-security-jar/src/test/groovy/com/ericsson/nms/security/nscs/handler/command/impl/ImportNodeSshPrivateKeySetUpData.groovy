...
class ImportNodeSshPrivateKeyHandlerSetupData extends CdiSpecification{

    @Inject
    SshPrivateKeyImportCommand command
    ...
    final String neType = "vECE"
    final String namespace = "NODE"
    final String platformType = "ESA"
    final String syncStatus = "syncStatus"
    final String synch = "UNSYNCHRONIZED"
    ...
    def setDataForNodeExists(final String nodeStatus, final String nodeName, final boolean fmAlarmState, final boolean isCliCmdSupported){
        ...
        switch(nodeStatus){
            case 'validNode' :
                setNodeData(nodeName, fmAlarmState, isCliCmdSupported)
                break
            case 'normNodeNull' :
     ...
     */
    def setNodeData(final String nodeName, final boolean fmAlarmState, final boolean isCliCmdSupported) {
        normNodeRef.getNormalizedRef() >> nodeRef
        NodeReference nodeReference = new NodeRef(nodeName)
        nscsMockCMReaderServ.getNormalizableNodeReference(nodeReference) >> normNodeRef
        nodeRef.getFdn() >> "NetworkElement=" + nodeName
        ...
        nscsCapabilityModelService.isNodeSshPrivateKeyImportSupported(normNodeRef) >> true
     ...
     */
    def setCommandDataForNodeSshPrivateKeyTxtFile(final String filePath, final String nodeName) {

        def fileContent = testSetupInitializer.getFileContent(filePath)
        commandMap.put(FileConstants.FILE_URI, fileContent)
        commandMap.put("sshprivatekeyfile",new String(fileContent))
        commandMap.put(Constants.FILE_PATH, filePath)
        commandMap.put(Constants.FILE_NAME,'nodeSshPrivateKeyFile.txt')
        commandMap.put("nodename", nodeName)
        NscsPropertyCommandInvoker commandInvokerValue = NscsPropertyCommandInvoker.CLI
        command.setCommandType(NscsCommandType.IMPORT_NODE_SSH_PRIVATE_KEY)
